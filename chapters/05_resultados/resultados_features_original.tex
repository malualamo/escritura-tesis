\section{Experimento 1: Resultados con variables en escala original}
\label{sec:resultados_features_original}

Previo al análisis, se descartaron cinco variables del conjunto 
...
que sigue se realizó sobre las 15 variables restantes.

\subsection{Diagnóstico de multicolinealidad (VIF)}

Para cada variable candidata se calculó el Variance Inflation 
...
Las 15 variables incluidas en el análisis presentaron valores de 
...
las variables. Este resultado indica que ninguna de las variables 
...
La figura~\ref{fig:delta_aic} presenta los valores de $\Delta$AIC obtenidos al incorporar individualmente cada variable en escala original al modelo base MD2. De las 15 variables evaluadas, 13 presentan valores negativos de $\Delta$AIC, lo que indica que su incorporación mejora el ajuste del modelo respecto al baseline. Las dos excepciones son \feat{ganancia\_informacion} y \feat{evidencia\_visual}, cuya incorporación empeora levemente el ajuste.

Las variables con mayor mejora corresponden a la categoría de información bottom-up: \feat{saliencia\_pixel} y \feat{saliencia\_media}. Dentro de las variables de estado interno del proceso bayesiano, \feat{entropia\_posterior} es la que mayor mejora aporta. En la categoría de calidad de la decisión, \feat{gap\_entropia} presenta el valor más negativo, mientras que \feat{distancia\_grilla} obtiene un valor moderado. Entre las variables de similitud con el target, \feat{similitud\_pixel} y \feat{similitud\_media} muestran mejoras de magnitud intermedia.

En el extremo inferior del ranking, \feat{competencia\_fp\_var} y \feat{posterior} muestran mejoras marginales.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{chapters/05_resultados/images/delta_aic_barras}
    \caption{$\Delta$AIC para variables en escala original, coloreadas por categoría y ordenadas por valor de $\Delta$AIC.}
    \label{fig:delta_aic}
\end{figure}
...
% \begin{tabular}{llr}
% \toprule
% \textbf{Categoría} & \textbf{Variable} & \textbf{$\Delta$AIC} \\
% \midrule
...
% \bottomrule
% \end{tabular}
% \caption{$\Delta$AIC para variables en escala original, ordenadas por categoría y % valor de $\Delta$AIC.}
% \label{tab:delta_aic_base}
% \end{table}
...
los modelos correspondientes a cada variable en escala original. De las 15 variables 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{chapters/05_resultados/images/resultados_TFCE_base}
...
La variable \feat{posterior} muestra una activación positiva que aparece alrededor de los 200 ms luego de la fijación, con distribución topográfica centro-parietal. La latencia y la distribución son consistentes con el componente P300. Una posible hipótesis es que cuando el modelo nnELM asigna alta probabilidad posterior a la celda fijada, el cerebro genera una respuesta tardía similar a la que se observa ante estímulos de relevancia para la tarea. 

Las variables \feat{saliencia\_media} y \feat{saliencia\_pixel} presentan el mismo patrón: una activación negativa con un pico consistente con el componente P100. Se observan algunas diferencias entre las variables: en \feat{saliencia\_media} se observa una activación de mayor amplitud y distribución fronto-central, mientras que en \feat{saliencia\_pixel} la topografía predominante es la occipital y la amplitud es menor. Una observación muy importante es que ambas variables exhiben activación significativa previo al onset de la fijación; para analizarlo, en futuros trabajos se podría abordar un análisis del modelado de la señal en relación a la sacada en lugar de la fijación.

La variable \feat{similitud\_pixel} presenta una activación negativa y un patrón más complejo para analizar. Se observan latencias tempranas y tardías, con una distribución que involucra electrodos fronto-centrales y parietales.

\begin{sloppypar}
Las 11 variables restantes no presentaron clusters significativos según el
criterio TFCE, entre ellas todas las variables de calidad de la decisión
